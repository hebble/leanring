...
public class RedisLearning {
    ...
    3.Redis为什么这么快？
       ...
       合理的线程模型
           ...
           单线程模型(Redis是单线程模型的，而单线程避免了CPU不必要的上下文切换和竞争锁的消耗)(网络模型: 单 Reactor 单线程模型)
       虚拟内存机制
    ...
    9.使用过Redis分布式锁嘛？有哪些注意点呢？
         ...
         但是存在问题:
           ...
           Redission 实现分布式锁的思想很简单，⽆论是主从集群还是 Redis Cluster 集群，它会对集群中的每个 Redis，挨个去执⾏设置 Redis 锁的脚本，也就是集群中的每个 Redis 都会包含设置好的锁数据。
