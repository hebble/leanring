        ...
        微服务应用中客户端如何感知配置中心的数据变化？
           1.4.x版本以后nacos客户端会基于长轮询机制从nacos获取配置信息，所谓的长轮询就是没有配置更新时，会在nacos服务端的队列进行等待
            nacos做了两个方面的优化:
                ...
                (2)分阶段进行比较和更新
                    a.第一阶段, 客户端把3000个配置项的key以及对应value的MD5值拼接成一个字符串, 然后发送到nacos server端进行判断, 服务端会逐个这些配置中MD5不同的key, 把存在更新的key返回给客户端
                    b.客户端拿到这些数据有变更的key, 循环逐个调用服务端, 从而获取这些Key对应的Value值
                这两个优化的核心目的是减少网络通信中数据包的大小, 相当于是把一次大数据包的通信拆分成了多个小数据包的通信, 虽然会增加网络通信的次数, 但是, 它提高了整体的数据传输性能, 最后加上长轮询的方式, 既
                减少了Pull轮询的次数, 又利用了长轮询的优势, 很好的实现了动态更新的同步功能
        默认是AP还是CP?如果切换成CP
