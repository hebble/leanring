...
public class MysqlLearning {
    /**
     ...
     4.数据库中的锁机制？
           ...
           4.3 行锁、间隙锁、next-key锁(临键锁) https://blog.csdn.net/weixin_43653890/article/details/124596746
           ...
           4.4 next-key锁(临键锁)的使用条件
                 MySQL锁是加在索引记录上面的。
                 (1)如果是非唯一性索引，不论表中是否存在该记录，除了会对该记录所在范围加锁，还会向右遍历到不满足条件的范围进行加锁。
                 (2)如果是唯一索引，如果表中存在该记录，只对该行记录加锁。如果表中不存在该记录，除了会对该记录所在范围加锁，还会向右遍历到不满足条件的范围进行加锁。
