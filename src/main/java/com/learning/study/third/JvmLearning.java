...
/**
 * https://blog.csdn.net/banzhuanhu/article/details/106871495 2020面试阿里字节跳动90%被问到的JVM面试题附答案
 * https://www.cnblogs.com/xfeiyun/p/15835733.html 四种引用类型: 强引用,软引用, 弱引用, 虚引用
 * https://blog.csdn.net/Lee_92/article/details/126170594 JVM相关工具的使用——JvisualVM
 * https://blog.csdn.net/qq_31960623/article/details/120541633 常见的几款JVM监控工具
 */
    ...
    /**
     ...
     2.详解JVM内存模型？
        ...
        jvm将虚拟机分为5大区域，程序计数器、虚拟机栈、本地方法栈、java堆、方法区；
             ...
             (5)方法区：存放已被加载的类信息、常量、静态变量、即时编译器编译后的代码数据。即永久代，在jdk1.8中不存在方法区了，被元数据区替代了，元方法区被分成两部分；1：加载的类信息，2：运行时常量池；加载的类信息被保存在元数据区中，运行时常量池保存在堆中；
     ...
     8.G1垃圾回收器
        8.1 G1介绍（Garbage first）(JDK1.9及其以后采取的默认垃圾回收器)
            具体可见图G1中Region分布示例图.jpg(region默认大小2M)
        ...
        8.2 G1的停顿时间控制(用户自定义停顿时间)（默认为200ms）
     ...
     15.项目中怎么优化JVM的?
        一个服务的Java参数配置：（服务器：Linux 64Bit，16Core×32G）
            JAVA_OPTS=“$JAVA_OPTS -server -Xmx20G -Xms20G -XX:NewRatio=3 -XX:SurvivorRatio=4 -Xss256k -XX:MaxTenuringThreshold=15 -XX:+AggressiveOpts -XX:+UseBiasedLocking -XX:+UseFastAccessorMethods
            -XX:+DisableExplicitGC -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:CMSFullGCsBeforeCompaction=1 -XX:+UseCMSCompactAtFullCollection
            -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=70 -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+PrintGCDateStamps -Xloggc:logs/gc-%t.log
     ...
     17.常见的几款JVM监控工具
        ...
        17.1 jps
            jps可以列出正在运行的Java进程，并显示虚拟机执行主类（Main Class，main()函数所在的类）名称以及进程id
                 ...
                 -v	程序启动时指定的jvm参数
        17.2 jstack
            jstack:打印线程快照, 查看某个Java进程中所有线程的状态。一般用来定位线程出现长时间停顿的原因，如发生死循环，死锁，请求外部资源长时间等待等！
            jstack pid
            图忽略
            从图中可以看到有很多线程，GC线程，myThreadA，myThreadB等，其中myThreadA和myThreadB的线程状态为BLOCKED，并且在最后提示出了死锁发生的位置
        17.3 jmap
            jmap:导出堆内存映像文件, jmap主要用来用来导出堆内存映像文件，看是否发生内存泄露等。
        ...
        17.4 jstat
            ...
            jstat查看一下类装载的信息。我个人很少使用这个命令，命令行看垃圾收集信息真不如看图形界面方便，就不多做介绍了。
             ...
             Time	花费的时间
        17.5 jinfo
            jinfo:实时查看和修改jvm配置参数, jinfo的作用是实时地查看和修改虚拟机各项参数。
